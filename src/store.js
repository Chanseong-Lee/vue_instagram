...
const store = createStore({
    state(){
        return {    //데이터 ( State )를 보관하는 공간
            dataList: dataList,
            more: {},
            btnCount: 0,
        }
    },
    mutations: {    //state를 변경하는 api 정의($store.state.변수명 으로 직접 수정하면 안됨)
        setLikes(state, idx){
    ...
    actions: {  // ajax요청을 하는 공간(mutations에서 ajax하면 안된다)
        getAdditionalData(context){
            if (this.state.btnCount < 2) {
                axios.get("https://codingapple1.github.io/vue/more" + this.state.btnCount + ".json")
                .then((result) => {
